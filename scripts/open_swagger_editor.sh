...
docker run --rm -d -p 8044:8080 --name "$name" -e SWAGGER_JSON=/openapi-definitions/openapi.yaml -v $(PWD)/openapi-definitions:/config swaggerapi/swagger-editor

wait_container_to_be_running "$name" & sleep 2

# open swagger-ui in browser
firefox --new-tab http://localhost:8044